...
import goNext from "./ts/transition";
import bodyValidator from "./ts/validator";
import createPublication from "./ts/createPublication";
import onChangeInput from "./ts/onChangeInput";
import searchPost from "./ts/searchPost";

const Dashboard = () => {
    const [posts, onLoadPost] = useState([]);
    const [isMobile, onScreenChange] = useState(window.innerWidth < 906);
    const [postInfo, onCreatePost] = useState({ title: '', body: '' });
    const [searchInfo, onSearch] = useState({ title: '', body: '' });
    const [currentPage, onChangePage] = useState(posts.length);
    ...
    const getPosts = async () => {
        const response = await fetch(`https://gorest.co.in/public/v2/users/${user.id}/posts`, {
            method: 'GET',
            headers: {
        ...
        onLoadPost(await response.json());
    }

    useEffect(() => {
        window.addEventListener('resize', () => {
            onScreenChange(window.innerWidth < 906);
        })
        getPosts();
    }, []);
    ...
    const sendRequest = async () => {
        ...
        const isValid = bodyValidator(payload.body);

        if(!isValid) return alert('Error en el largo del texto')
    ...
    const deleteHandler = async (id: number) => {
        const accept = window.confirm('¿Quiere eliminar esta publicación?');
    ...
    return(
        <div className='dashboard-wrapper'>
            <div className='nav-wrapper'>
                <h1 className='welcome-title-nav'>
                    { isMobile ? '' : 'Hola'} {user.name}
                </h1> 
            ...
            <div className='main-content'>
                <div className='search-container'>
                    <div className='search-text'>Busque su publicación aquí</div>
                    <div className='search-options'>
                        <input onChange={evt => onChangeInput(evt, 'title', onSearch)} value={searchInfo.title} className='search-field' placeholder='Busqueda por título'/>
                        <input onChange={evt => onChangeInput(evt, 'body', onSearch)} value={searchInfo.body} className='search-field' placeholder='Busqueda por contenido'/>
                    </div>

                    <button onClick={() => searchPost(searchInfo, posts, onLoadPost, onSearch, onChangePage, currentPage)} className='search-button'>Buscar</button>
                    <button onClick={() => getPosts()} className='search-button'>Limpiar Filtro</button>
                </div>
                    ...
                    <div className='section-01'>
                        <div className="carousel-outer">
                            <div id='posts-carousel'>
                                {
                                    posts.map((post: { title : string, body: string, id: number }, index) => (
                        ...
                        <div className='button-menu'>
                            <button className='paginator-button' onClick={() => goNext(currentPage, posts.length, onChangePage, 'prev')}>{'←'}</button>
                            <button className='paginator-button' onClick={() => goNext(currentPage, posts.length, onChangePage, 'next')}>{'→'}</button>
                        </div>
                    </div>
                        ...
                        <div className='new-post-container'>
                            <div className='new-post-content'>
                                ...
                                <div className='post-section'>
                                    <input value={postInfo.title} className='post-title-input' placeholder='Título de su publicación' onChange={evt => onChangeInput(evt, 'title', onCreatePost)}/>
                                    <textarea placeholder='Texto aquí' value={postInfo.body} className='textarea-field' onChange={evt => onChangeInput(evt, 'body', onCreatePost)}/>
                                </div>
                            </div>
                            <button className='post-button' onClick={() => createPublication(sendRequest, postInfo, posts, currentPage, onChangePage)}>Publicar</button>
                        </div>
                    </div>
                </div>
            </div>
